...
sawmill_logger_record_sleep(struct timespec time_awake) {
    ...
    sMSUntilPrint = CLAMP(PRINT_INTERVAL_MS - (time_now_ms() - sTimeOnPrint), 0, PRINT_INTERVAL_MS);
}
...
void read_proc_loadavg()
{
	...
	g_message("loadavg:1m:%s:5m:%s:15m:%s kr/ke:%s pid:%s", arr_file[0], arr_file[1], arr_file[2], arr_file[3], arr_file[4]);
...
void read_proc_net_dev()
{
	...
	for(n=2; n < newlines ; n++){
		...
		if((!g_strcmp0(g_strstrip(arr_line[0]),"eth0")) || (!g_strcmp0(g_strstrip(arr_line[0]),"ppp0"))){
			...
			while(arr_fields[k]){
				...
				if(nth_field == 1) received_packets = k;
				if(nth_field == 9) transmitted_packets = k;
				++k;
				++nth_field;
			}
...
sawmill_logger_record_wake(struct timespec time_asleep) {
    ...
    //TODO: use g_timer_source_set_interval(GTimerSource *tsource, guint interval_ms, gboolean from_poll)
    g_source_remove(sTimerEventSource);
    sTimerEventSource = g_timeout_add_full(G_PRIORITY_DEFAULT, CLAMP(sMSUntilPrint - ms_asleep, 0, PRINT_INTERVAL_MS), sawmill_logger_update, GINT_TO_POINTER(FALSE), NULL);
}
